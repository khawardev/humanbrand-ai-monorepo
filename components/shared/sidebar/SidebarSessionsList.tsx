...
export function SidebarSessionsList({ savedSessions }: SidebarSessionsListProps) {
	...
	// Filter sessions
	const chatSessions = savedSessions?.filter((session: any) => session.sessionType === 'ai_chat') || []
	const otherSessions = savedSessions?.filter((session: any) => session.sessionType !== 'ai_chat') || []
