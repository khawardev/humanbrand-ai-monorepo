...
import { CircleXIcon, ListFilterIcon, TrashIcon } from "lucide-react";
...
export function DataTableToolbar<TData>({
  ...
  deleteConfirmMessage = (count) =>
    `This action cannot be undone. This will permanently delete ${count} selected item(s).`,
  topBarComponents,
}: DataTableToolbarProps<TData>) {
  const id = React.useId();
  const [primarySearchValue, setPrimarySearchValue] = React.useState("");
  const primarySearchColumnId =
    searchableColumns.length > 0 ? searchableColumns[0].id : undefined;
  ...
  React.useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      table
        .getColumn(primarySearchColumnId as string)
        ?.setFilterValue(primarySearchValue);
    }, 300);
  ...
  return (
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      <div className="flex flex-row items-center sm:items-center gap-2 grow">
