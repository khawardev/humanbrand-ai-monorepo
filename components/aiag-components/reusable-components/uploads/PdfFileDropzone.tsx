...
type PdfFileDropzoneProps = {
    ...
    setFiles: (files: File[]) => void;
    maxFiles?: number;
    setReferenceMaterial: (data: any) => void;
};
...
export function PdfFileDropzone({ files, setFiles, maxFiles = 4, setReferenceMaterial }: PdfFileDropzoneProps) {
    const [pdfLoading, setPdfLoading] = useState(false)
    ...
    const onDrop = useCallback(
        (acceptedFiles: File[], fileRejections: FileRejection[]) => {
            ...
            if (fileRejections.length > 0) toast.error("Only PDF files are accepted.");
        ...
        [files, setFiles, maxFiles, setReferenceMaterial]
    );
    ...
    const { getRootProps, getInputProps, isDragActive } = useDropzone({ onDrop, multiple: maxFiles > 1, accept: { "application/pdf": [".pdf"] } });
    ...
    return (
        <div className="w-full">
            ...
            >
                ...
                <MdOutlineFileUpload className="w-12 h-12 text-muted-foreground/50" />
                <p className="mt-2 px-10 text-sm text-center text-muted-foreground">
                    {isDragActive ? "Drop the PDF files here..." : `Drag & drop PDFs here, or click to select (Max: ${maxFiles})`}
                </p>
            </div>
            ...
            <div className="flex flex-col gap-4 mt-4 w-full">
                {pdfLoading ? (
                    ...
                    files.map((file, index) => (
                        <div key={file.name + index} className="relative flex items-center justify-between p-2 pl-4 border rounded-md bg-accent">
                            <div className="flex items-center gap-4 flex-grow min-w-0">
                                <PiFilePdfFill className="h-6 w-6 text-muted-foreground flex-shrink-0" />
                                <div className="flex flex-col min-w-0">
                            ...
                            <Button type="button" variant="ghost" size="icon" className="ml-2 h-7 w-7 flex-shrink-0" onClick={() => removeFile(index)}>
                                <IoClose className="h-4 w-4" />
                            </Button>
                        </div>
                    ))
