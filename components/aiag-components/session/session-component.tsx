...
// const ToneAndCreativitySection = dynamic(() => import('@/components/aiag-components/selection-components/ToneAndCreativitySection').then(mod => mod.ToneAndCreativitySection))
...
// 1. Updated the destructured variables from `useSessionContentGenerator` to use the new names (`referenceFileInfos`, `handleReferenceFileChange`, `chatFileInfos`, `handleChatFileChange`).
// 2. Modified the props passed to `ReferenceMaterialSection` to use `initialFileInfos` and `onFilesChange`.
// 3. Updated the props for `GeneratedContent` to pass `chatFileInfos` and `onChatFileChange` for use in the chat component.
