...
export function getRevisionPrompts(
    ...
    const originalContent = sessionData.revised_content || sessionData.original_content || "";
...
* Audience: ${params.selected_audience?.join(', ') || 'N/A'}
* Focus: ${params.selected_focus || 'N/A'}
* Types: ${params.selected_content_types?.join(', ') || 'N/A'}
* Platforms: ${params.selected_platforms?.join(', ') || 'N/A'}
* CTAs: ${params.selected_ctas?.join(', ') || 'N/A'}
* Original Instructions: ${params.custom_prompt_instructions || 'None'}
---
...
export function getHyperRelevancePrompts(
    sessionData: any,
    hyperRelevance: { personasText: string; uploadedFilesData: string },
    knowledgeBaseContent: string
): any {
    const originalContent = sessionData.revised_content || sessionData.original_content || "";
...
`;
    ...
    const userPrompt = `
Original AIAG Content (to be adapted):
${originalContent}
---
Target Persona(s) Description (Manual Input - General Guidance):
${hyperRelevance.personasText || "None Provided. Check KB for standard personas if applicable."} (Refer to the KB Section 3.11 for persona language hints).
---
Target Persona(s) Details (from Uploaded Files - ***PRIORITIZE THIS DATA***):
${hyperRelevance.uploadedFilesData || "No specific file data provided. Rely on manual description or KB personas if applicable."}
---
