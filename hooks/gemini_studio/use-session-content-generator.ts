...
import { updateSessionContent, adaptPersonaForSession, manageImageForSession, updateChatForSession } from "@/actions/session-actions"
...
export function useSessionContentGenerator(initialData: any) {
    ...
    const handleImageAction = (newPrompt: string, imageFile: File | null) => {
        startImageTransition(async () => {
            ...
            if (result.success && result.imageUrl) {
                ...
                const imageRefInfo = imageFile ? { name: imageFile.name, size: imageFile.size, type: imageFile.type } : null;

                await manageImageForSession(initialData.id, {
                    imagePrompt: newPrompt,
                    imageUrls: newImageUrls,
                    imageReferenceFileInfo: imageRefInfo,
                });
    ...
    const handleChatSend = async (userInput: string, chatPdfInfo: any) => {
        ...
        const systemPrompt = getChatSystemPrompt({
            ...
            uploadedFileText: chatPdfInfo?.parsedText,
        ...
        await updateChatForSession(initialData.id, {
            ...
            chatPdfInfo: chatPdfInfo?.fileInfo,
        });
    };
