...
// 1. Renamed `referencePdfInfo` to `referenceFileInfos` and `referencePdfData` to `referenceFilesData` for clarity and to reflect multi-file support.
// 2. Initialized `referenceFileInfos` as an empty array `[]`.
// 3. Updated `handleReferenceFileChange` to handle the new data structure from `FileDropzone`, which now provides `fileInfos` (an array) and `parsedText`.
// 4. Modified the `sessionData` object to save `referenceFileInfos` to the database, ensuring persistence of multiple file metadata.
