...
export function useKnowledgeBaseChat(initialData: { user: any; initialChatHistory: any[]; sessionId?: string; retrievalStrategy?: 'full-context' | 'rag' }) {
    ...
    const retrievalStrategy = initialData.retrievalStrategy === 'rag' ? 'rag' : 'knowledgeBase';
    ...
    const handleSendMessage = useCallback(async (userInput: string, attachments: File[] = []) => {
        ...
        if ((!userInput.trim() && attachments.length === 0) || isResponding) return;
        ...
        const optimisticHistory = [...chatHistory, { 
            role: 'user', 
            content: userInput, 
            images: processedAttachments.length > 0 ? processedAttachments : undefined
        }];
        setChatHistory(optimisticHistory);

        try {
            // 1. Save User Message
            const result = await saveUserMessage(sessionId || null, user.id, userInput, chatHistory, processedAttachments);
            ...
            if (result.success && result.sessionId) {
                ...
                // We do NOT call triggerResponse here explicitly anymore, 
                // because setting chatHistory will trigger the useEffect 
                // that watches [chatHistory, sessionId, user, isResponding].
                // However, we must ensure isResponding is FALSE so the effect runs.
                setIsResponding(false);
