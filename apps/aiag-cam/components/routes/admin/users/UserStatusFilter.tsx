...
interface UserStatusFilterProps<TData> {
  table: Table<TData>;
}

export function UserStatusFilter<TData>({
  table,
}: UserStatusFilterProps<TData>) {
  ...
  const handleCheckedChange = (checked: boolean, value: boolean) => {
    const currentFilter = statusColumn?.getFilterValue() as
      | boolean[]
      | undefined;
